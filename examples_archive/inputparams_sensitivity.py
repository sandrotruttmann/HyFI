...
fig, axs = plt.subplots(nrows=2, ncols=1,
                        figsize=(115*mm, 230*mm))
plt.rcParams.update({'font.size': 10})


# Define a list of linestyles to distinguish r_nn values
linestyles = ['-', '--', '-.', ':', (0, (3, 5, 1, 5)), (0, (5, 10))]  # Add more if needed

# Generate a colormap for dt_nn
cmap_dt_nn = mcp.gen_color(cmap='viridis', n=len(dt_nn_list))

for r_nn_idx, r_nn in enumerate(r_nn_list):
    for dt_nn_idx, dt_nn in enumerate(dt_nn_list):
        now = datetime.now()
...
axs[0].set_xlabel('Angular difference [°]')
axs[0].set_ylabel('Cumulative frequency')
axs[0].set_xlim([0, 70])
axs[0].legend(prop={'size': 6})

axs[1].set_xlabel('Search radius rnn [m]')
axs[1].set_ylabel('Search time dt_nn [h]')
