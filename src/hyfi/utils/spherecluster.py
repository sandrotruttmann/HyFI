...
# -*- coding: utf-8 -*-
"""
Internal implementation of spherical clustering algorithms.

This module provides spherical K-means and von Mises-Fisher mixture clustering
algorithms, adapted from the spherecluster package with corrections.

Based on:
- spherecluster package with fixes from GitHub PR #34
- movMF R package for von Mises-Fisher distributions
"""
...
# Handle different sklearn versions for _kmeans_plusplus
try:
    from sklearn.cluster._kmeans import _kmeans_plusplus
except ImportError:
    try:
        from sklearn.cluster._k_means import _kmeans_plusplus
    except ImportError:
        # Fallback for older sklearn versions
        from sklearn.cluster.k_means_ import _k_means_plus_plus as _kmeans_plusplus
...
# ============================================================================
# von Mises-Fisher Mixture Implementation
# ============================================================================
...
def _vmf_labels_inertia(X, centers):
...
def _vmf_log(X, kappa, mu):
    """Computes log(vMF(X, kappa, mu)) using built-in numpy/scipy Bessel
    approximations.
...
def _init_unit_centers(X, n_clusters, random_state, init):
    ...
    if isinstance(init, np.ndarray):
        ...
        labels, inertia, centers, iters = _spherical_kmeans_single_lloyd(
...
class VonMisesFisherMixture(KMeans):
    ...
    posterior_type: 'soft' or 'hard'
        Type of posterior computed in expectation step.
    ...
    posterior_ : array, [n_clusters, n_examples]
        Each column corresponds to the posterior distribution for an example.
    ...
    def predict(self, X):
        ...
        X = self._check_test_data(X)
        return _vmf_labels_inertia(X, self.cluster_centers_)[0]
    ...
    def score(self, X, y=None):
        ...
        check_is_fitted(self)
        X = self._check_test_data(X)
        return -_vmf_labels_inertia(X, self.cluster_centers_)[1]
